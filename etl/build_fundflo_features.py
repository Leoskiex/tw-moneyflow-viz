...
ETF_HOLDINGS = ROOT / "data" / "etf"  # multi-ETF: scan */holdings (AISTOCKMAP)
...
def load_etf_share_deltas() -> Dict[str, Dict[str, float]]:
    """date -> {code: share_delta} aggregated across data/etf/*/holdings snapshots.

    Diffs consecutive dated holdings JSON by `share` (not precomputed share_delta).
    Multiple active ETFs sum into the same date/code bucket for etf_flow_yi.
    Path convention (AISTOCKMAP): data/etf/<code>/holdings/YYYY-MM-DD.json
    """
    by_date: Dict[str, Dict[str, float]] = defaultdict(lambda: defaultdict(float))
    etf_root = ROOT / "data" / "etf"
    if not etf_root.is_dir():
        return {}

    scanned = []
    for etf_dir in sorted(etf_root.iterdir()):
        if not etf_dir.is_dir():
            continue
        holds_dir = etf_dir / "holdings"
        ...
        if len(dated) < 2:
            # single snapshot cannot form a delta; still record scan
            if dated:
                scanned.append(etf_dir.name)
            continue
        prev_map: Dict[str, float] = {}
        prev_dt: Optional[str] = None
        for dt, path in dated:
            ...
            for h in doc.get("holdings") or []:
                ...
                if not code:
                    continue
                try:
                    cur_map[code] = float(h.get("share") or 0.0)
                except (TypeError, ValueError):
                    continue
            if prev_dt is not None:
                codes = set(prev_map) | set(cur_map)
                for code in codes:
                    delta = cur_map.get(code, 0.0) - prev_map.get(code, 0.0)
                    if delta:
                        by_date[dt][code] += delta
            prev_map = cur_map
            prev_dt = dt
            scanned.append(etf_dir.name)
    ...
    out: Dict[str, Dict[str, float]] = {
        dt: {c: float(v) for c, v in mp.items()} for dt, mp in by_date.items()
    }
    return out
...
def build(raw_dir: Optional[Path], max_dates: Optional[int] = None) -> Tuple[dict, List[str]]:
    ...
    latest = {
        "meta": {
            ...
            "etf_note": "etf_flow_yi = sum over data/etf/*/holdings consecutive share deltas * price / 1e8 (active ETFs)",
